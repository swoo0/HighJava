...
public class T02_FIle {
	...
	public static void displayFileList(File dir) {
		...
		// 하위 디렉토리 정보를 저장할 ArrayList 생성(File배열의 인덱스 정보 저장)
		...
		for(int i = 0; i < files.length; i++) {
			...
			if (files[i].isDirectory()) {
				...
				size = files[i].length() + "";
				attr = files[i].canRead() ? "R" : " ";		// canRead() 읽기 권한 확인
				attr += files[i].canWrite() ? "W" : " ";	// canWrite() 쓰기 권한 확인
				attr += files[i].isHidden() ? "H" : " ";    // isHidden() 히든 파일 인지 확인
			}
		...
		for (int i = 0; i < subDirList.size(); i++) {
			// 하위 폴더의 내용들도 출력하기 위해 현재 메서드를 재귀 호출하여 관리한다.
