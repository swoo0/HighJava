...
@WebServlet("/Servlet")
public class T02_Servlet extends HttpServlet {
/*
	서블릿 동작 방식에 대하여...
	
	1. 사용자(클라이언트)가 URL을 클릭하면 HTTP Request를 서블린 컨테이너로 전송(요청)한다.
	2. 컨테이너는 web.xml에 정의된 url패턴을 확인하여 어느 서블릿을 통해 처리해야 하는지는 검색한다.
		(로딩이 안된 경우에는 로딩함. 로딩시 init()메서드 호출됨.)
	3. 서블릿 컨테이너는 요청을 처리할 개별 스레드 객체를 생성하여 해당 서블릿 객체의
		service() 메서드를 호출한다.
		(HttpServleteRequest 및 HttpSercletResponse객체를 생성하여 파라미터로 넘겨준다.)
...
*/	
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 반드시 request에서 값을 가져오기 전에 먼저 설정홰야함.
		...
		// 응답메시지 인코딩 설정
		// (Content-Type의 ;charset=UTF-8 과 동일)
