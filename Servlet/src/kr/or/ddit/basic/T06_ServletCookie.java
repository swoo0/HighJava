...
public class T06_ServletCookie extends HttpServlet {
/*
	쿠키란 ? 
...
//		readCookieExam(req, resp); // 쿠키정보 읽기 예제
		...
		deleteCookieExam(req, resp); // 쿠기정보 삭제 예제
	}
	...
	private void readCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (cookies != null) {
			...
			for (Cookie cookie : cookies) {
				...
				out.println("value : " + URLEncoder.encode(cookie.getValue(), "UTF-8") + "<br>");
				out.println("<hr>");
			}
		} else {
			out.println("<h2>쿠키정보가 없습니다. </h2>");
	...
	/*
		...
		1. 쿠키객체를 생성한다. 사용불가 문자 ( 공백, [], () = , " / ? @ : ; )
		  Cookie cookie = new Cookie("키값", "value값");
		   쿠키값은 사용불가 문자를 제외한 나머지 출력가능한 아스키 문자 사용 가능함.
